...
(function (global) {
  ...
  function farkGoster(dogru, cevap) {
    ...
    p.forEach(function (o) {
      if (o.t === '=') g.appendChild(document.createTextNode(o.s + ' '));
        ...
        /* kabul edilen karşılık: hata değil, ama hedefteki biçim de gösterilir */
        g.appendChild(e('mark', { title: 'Kabul edildi · hedef cümlede: ' + o.beklenen }, o.s));
  ...
  /* ───── ustalık şeridi ─────────────────────────────────────── */
  function masteryCubuk(oge, secenek) {
    secenek = secenek || {};
    var s = Mastery.al(oge);
    var kap = e('div', { class: 'mastery-serit', style: secenek.stil || '' });
    if (!secenek.sade) {
      var et = Mastery.etiket(s.genel);
      kap.appendChild(e('div', { style: 'display:flex;align-items:center;gap:8px;margin-bottom:9px' }, [
        e('span', { style: 'font-size:14px;color:' + et.renk }, et.ikon),
        e('b', { style: 'flex:1;font-size:12px;font-weight:800;letter-spacing:.1em;text-transform:uppercase;color:var(--ink-3)' }, 'Ustalık'),
        e('span', { style: 'font-size:12.5px;font-weight:800;color:' + et.renk }, et.ad + ' · %' + s.genel)
      ]));
    }
    var iz = e('div', 'mastery-izgara');
    Mastery.BECERILER.forEach(function (b) {
      var v = s[b] || 0;
      iz.appendChild(e('div', {
        class: 'mastery-hucre', title: Mastery.beceriAdi(b) + ' · %' + v + ' — ' + Mastery.beceriAciklama(b)
      }, [
        e('div', 'mastery-dolgu', [e('i', { style: 'height:' + Math.max(3, v) + '%' })]),
        e('span', null, Mastery.beceriAdi(b).slice(0, 3))
      ]));
    });
    kap.appendChild(iz);
    return kap;
  ...
  /* ───── beceri radarı ──────────────────────────────────────── */
  function beceriRadar(ozet) {
    var ns = 'http://www.w3.org/2000/svg';
    var B = Mastery.BECERILER, n = B.length;
    var boy = 260, mrk = boy / 2, r = boy / 2 - 42;
    var svg = document.createElementNS(ns, 'svg');
    svg.setAttribute('class', 'radar');
    svg.setAttribute('viewBox', '0 0 ' + boy + ' ' + boy);
    function nokta(i, oran) {
      var a = (Math.PI * 2 * i / n) - Math.PI / 2;
      return [mrk + Math.cos(a) * r * oran, mrk + Math.sin(a) * r * oran];
    }
    /* ağ halkaları */
    [0.25, 0.5, 0.75, 1].forEach(function (h) {
      var p = document.createElementNS(ns, 'polygon');
      p.setAttribute('class', 'ag');
      p.setAttribute('points', B.map(function (_, i) { return nokta(i, h).join(','); }).join(' '));
      svg.appendChild(p);
    });
    /* eksenler */
    B.forEach(function (_, i) {
      ...
      var d = nokta(i, 1);
      l.setAttribute('class', 'ag');
      l.setAttribute('x1', mrk); l.setAttribute('y1', mrk);
      l.setAttribute('x2', d[0]); l.setAttribute('y2', d[1]);
      svg.appendChild(l);
    });
    /* değer alanı */
    var alan = document.createElementNS(ns, 'polygon');
    alan.setAttribute('class', 'alan');
    alan.setAttribute('points', B.map(function (b, i) {
      return nokta(i, Math.max(0.03, (ozet.beceri[b] || 0) / 100)).join(',');
    }).join(' '));
    alan.style.transformOrigin = mrk + 'px ' + mrk + 'px';
    svg.appendChild(alan);
    /* etiketler */
    B.forEach(function (b, i) {
      var d = nokta(i, 1.24);
      var t = document.createElementNS(ns, 'text');
      t.setAttribute('x', d[0]); t.setAttribute('y', d[1]);
      t.setAttribute('text-anchor', d[0] < mrk - 6 ? 'end' : d[0] > mrk + 6 ? 'start' : 'middle');
      t.setAttribute('dominant-baseline', 'middle');
      t.textContent = Mastery.beceriAdi(b) + ' %' + (ozet.beceri[b] || 0);
      svg.appendChild(t);
    });
    return svg;
  }
  ...
  global.UI = {
    ...
    masteryCubuk: masteryCubuk, beceriRadar: beceriRadar
  };
})(window);
