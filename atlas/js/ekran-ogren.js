...
(function (global) {
    ...
    function ciz() {
      Veri.modulListesi(seciliSeviye).then(function (mods) {
        ...
        mods.forEach(function (m, i) {
          ...
          var sinavNotu = Atlas.oku('sinav:' + m.f, null);
          d.appendChild(e('div', {
            class: 'modul',
            ...
            /* modül sınavı — bir kez çalışılmışsa açılır */
            p.n > 0 ? e('button', {
              class: 'dg kucuk sade',
              title: sinavNotu ? 'Son sınav: %' + sinavNotu.yuzde + ' (' + sinavNotu.tarih + ')' : 'Modül sınavına gir',
              style: sinavNotu ? 'color:' + (sinavNotu.yuzde >= 70 ? 'var(--ok)' : 'var(--warn)') : '',
              onclick: function (ev) { ev.stopPropagation(); Uygulama.git('#/sinav/' + m.f); }
            }, sinavNotu ? '📝 ' + sinavNotu.yuzde : '📝') : null,
            e('div', 'yuzde', p.oran + '%')
          ]));
  ...
  function oturumBaslat(g, ayar) {
    ...
    ust.appendChild(e('button', {
      class: 'dg yuvarlak sade', title: 'Çıktı al',
      onclick: function () { Cikti.menu(liste, ayar.baslik || 'Çalışma listesi'); }
    }, '🖨'));
    g.appendChild(ust);
    ...
    function ciz() {
      if (i >= liste.length) { OturumHafiza.temizle(); bitir(); return; }
      var c = liste[i];
      /* yarıda bırakılırsa kaldığı yerden devam edebilsin */
      if (liste.length > 3) {
        OturumHafiza.kaydet({
          idler: liste.map(function (x) { return x.id; }).filter(Boolean),
          i: i, kip: kip, kaynak: ayar.kaynak, geriYol: ayar.geriYol
        });
      }
      ...
      if (kip === 'tanima') {
        ...
        /* görsel bağlam: kelimeyi çeviriye değil sahneye bağlar */
    ...
    function sonucGoster(c, kart, skor, cevap, hedef, pasGec, kendiPuan) {
      ...
      Atlas.cevapla({
        tip: 'c', id: c.id, dogruMu: basarili, skor: skor, kip: kip,
      ...
      if (!kendiPuan) {
        ...
        /* Farklı yazdı ama tam puan aldıysa nedenini söyle — yoksa
           öğrenci "yanlış yazdım ama neden %100 aldım" diye kalır. */
        if (cevap && !pasGec && skor === 100) {
          var neden = Atlas.esdegerNedeni(hedef, cevap);
          if (neden) alt.appendChild(e('div', 'esdeger-not', [
      ...
      /* ustalık şeridi — bu cümleyi hangi beceriyle biliyor */
      alt.appendChild(UI.masteryCubuk('c:' + c.id));

      /* doğru cümle */
      var dogruKart = e('div', { class: 'kart', style: 'margin-top:10px;text-align:center' });
      if (kip !== 'uretim') dogruKart.appendChild(Gorsel.kutu(c));
    ...
    function bitir() {
      ...
      UI.kutla({
        ...
        dugmeler: [
          ...
          /^modul:/.test(ayar.kaynak || '')
            ? { ad: '📝 Modül sınavına gir', fn: function () { Uygulama.git('#/sinav/' + ayar.kaynak.slice(6)); } }
            : null,
          yanlis > 0
            ? { ad: '🏋️ Yanlışları antrenman et', fn: function () { Uygulama.git('#/antrenman'); } }
