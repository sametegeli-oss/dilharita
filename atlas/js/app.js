...
(function (global) {
  ...
  Ekran.ev = function (g) {
    ...
    /* ── yarım kalan oturum ── */
    var devam = OturumHafiza.devamKarti();
    if (devam) { devam.style.marginTop = '14px'; g.appendChild(devam); }
    ...
    /* ── beceri profili ── */
    var mOzet = Mastery.ozet();
    if (mOzet.oge >= 5) {
      g.appendChild(e('div', 'bolum-ad', 'Beceri profilin'));
      var zayif = Mastery.profilZayifi();
      g.appendChild(e('div', { class: 'kart gir gir-2' }, [
        UI.beceriRadar(mOzet),
        e('div', { class: 'kucuk-yazi', style: 'margin-top:10px;text-align:center' },
          zayif ? 'En zayıf becerin: ' + Mastery.beceriAdi(zayif).toLowerCase() +
            '. ' + Mastery.beceriAciklama(zayif) + ' — akıllı tekrar bunu hedefler.'
            : mOzet.oge + ' öğe üzerinden hesaplandı.'),
        e('button', {
          class: 'dg tam', style: 'margin-top:10px',
          onclick: function () { Uygulama.git('#/akilli'); }
    ...
    var gruplar = [
      {
        ad: 'Öğren', oge: [
          ...
          ['#/yol', '🧭', 'Öğrenme yolu', 'Nereden geçtin, sırada ne var'],
          ['#/ders', '🎓', 'Bugünkü dersim', 'Öğretmenin hazırladığı 5 adımlı ders'],
          ['#/hikaye', '📻', 'Modül hikâyesi', 'Cümleler bir hikâyede · podcast'],
          ['#/telafi', '🩹', 'Telafi', 'Alt seviyede kalan boşluklar']
        ]
      },
      {
        ad: 'Pratik', oge: [
          ['#/tekrar', '🔁', 'Tekrar', 'Vadesi gelen cümle ve kelimeler'],
          ['#/akilli', '🧠', 'Akıllı tekrar', 'Ne çalışacağına motor karar versin'],
          ['#/antrenman', '🏋️', 'Hata antrenmanı', 'Üç aşamalı mikro döngü'],
          ...
          ['#/konusma', '💭', 'Günün konuşması', 'Üç soru · kalıplar · sesli cevap'],
          ['#/gunsonu', '🌙', 'Gün sonu', 'Bugünün karma tekrarı']
        ]
      },
      {
        ad: 'Okuma ve ses', oge: [
          ['#/kutuphane', '📚', 'Kütüphane', 'Gutenberg · telifsiz kitaplar'],
          ['#/foto', '📷', 'Fotoğraftan ekle', 'Sayfayı tara, cümleleri topla'],
          ['#/pdf', '📄', 'PDF oku', 'Ders kitabı, makale, not'],
          ['#/studyo', '🌊', 'Ses dalga stüdyosu', 'Hoca çizdir → sen oku → kıyasla'],
          ['#/akustik', '📉', 'Akustik artikülasyon', 'Dalga, spektrum, ünlü haritası'],
          ['#/video', '📺', 'Video pratik', 'Kelimeyi gerçek konuşmalarda duy']
        ]
      },
      {
        ad: 'İlerleme', oge: [
          ...
          ['#/seri', '🔥', 'Öğrenme çizgin', 'Seri, kilometre taşları, koruma'],
          ['#/seviye-testi', '📝', 'Seviye testi', 'Kısa testle seviyeni ölç']
        ]
      },
      {
        ad: 'Ayarlar', oge: [
          ['#/ayarlar', '⚙️', 'Ayarlar', 'Tema, ses, görsel, hatırlatma, AI'],
          ['#/anayasa', '📜', 'Öğretmen anayasası', 'Öğretmenin kalıcı kuralları'],
          ['#/kible', '🕋', 'Namaz vakitleri ve kıble', 'Yerel hesap, yıllık tablo'],
          ['#/hesap', '☁️', 'Hesap ve bulut', 'İki cihaz arası senkron'],
          ['#/ses-teshis', '🔬', 'Ses teşhis', 'Hangi ses ne yapıyor'],
          ['#/veri', '💾', 'Veri ve yedek', 'Yedek al, geri yükle, aktar'],
          ['#/kilavuz', '📘', 'Kullanım kılavuzu', 'Hangi ekran ne işe yarar'],
  ...
  function kabukKur() {
    ...
    /* koç düğmesi — her ekranda ulaşılabilir */
    document.body.appendChild(e('button', {
      class: 'koc-dugme', title: 'Koça sor',
      onclick: function () { if (global.KocBalon) KocBalon.sor(); }
    }, '🧑‍🏫'));
  ...
  function baslat() {
    ...
    if (global.Hatirlatma) Hatirlatma.zamanlayiciKur();

    /* bulut yapılandırılmışsa sessizce bağlan */
    if (global.Bulut && Bulut.yapilandirildiMi()) {
      Bulut.yukle().then(function () {
        setTimeout(function () {
          if (Bulut.kullanici && Atlas.Ayar.al().otoSenkron !== false) {
            Bulut.tamSenkron().then(function () {
              UI.bildir('Bulutla senkronize edildi', 'ok', 2200);
              Uygulama.ustYenile();
            }).catch(function () {});
          }
        }, 1200);
