...
class PhotoAvatar{
  ...
  blink(){
    ...
    const beforeSpeaking = State.speaking;
    this.show(this.frames.blink);
    setTimeout(()=>{
      this.isBlinking=false;
      if(State.speaking){
        this.show(this.frames.mouthSmall);
      }else{
        this.show(this.frames.idle);
      }
      this.scheduleBlink();
  ...
  speak(duration){
    ...
    this.talkTimer=setInterval(()=>{
      ...
      this.show(seq[this.talkIndex % seq.length]);
      this.talkIndex++;
    }, 115);
...
function pickVoice(){
  ...
  const maleRe = /(male|david|mark|george|daniel|james|john|alex|fred|thomas|guy|brian|ryan|matthew)/i;
  const femaleRe = /(female|zira|jenny|aria|samantha|susan|hazel|victoria|karen|moira|tessa|joanna|salli|amy|emma|olivia)/i;
...
function speakText(text){
  ...
  try{
    ...
    u.rate = .96;
    if(String(Scenario.voiceGender || "").toLowerCase() === "male"){
      u.pitch = .82;
    }else{
      u.pitch = 1.08;
    }
