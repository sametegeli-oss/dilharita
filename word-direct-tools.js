...
(function() {
  ...
  function wdGetPrompt(kind) {
    try { return localStorage.getItem(wdPromptKey(kind)) || WD_PROMPT_DEFAULTS[kind] || WD_PROMPT_DEFAULTS.teacher; } catch (e) { return WD_PROMPT_DEFAULTS[kind] || WD_PROMPT_DEFAULTS.teacher; }
  }
  ...
  // DÜZELTİLDİ: speakMixed içinde speak yerine doğrudan SpeechSynthesis kullan
  function speakMixed(text) {
    if (window.DH_speakMixed && window.DH_speakMixed !== speakMixed) {
      return window.DH_speakMixed(text);
    }
    try {
      ...
      function next() {
        ...
        u.lang = c.lang;
        u.rate = c.lang === "tr-TR" ? 0.96 : 0.88;
        u.onend = next;
        speechSynthesis.speak(u);
      }
      next();
    } catch (e) {
      // Fallback: doğrudan ses çal
      try {
        speechSynthesis.cancel();
        const u = new SpeechSynthesisUtterance(String(text));
        u.lang = isTurkishChunk(text) ? "tr-TR" : "en-US";
        u.rate = 0.9;
        speechSynthesis.speak(u);
      } catch (e2) {
        console.warn("[wd] Ses çalınamadı:", e2);
      }
    }
  }
  ...
  // DÜZELTİLDİ: currentData düzgün kapatıldı
  ...
  // DÜZELTİLDİ: openGoogleTranslate currentData'dan ayrıldı
  function openGoogleTranslate() {
    ...
    const url = "https://translate.google.com/?sl=en&tl=tr&text=" + encodeURIComponent(d.sentence) + "&op=translate";
  ...
  function speak(text, lang, rate) {
    try { speechSynthesis.cancel(); const u = new SpeechSynthesisUtterance(text); u.lang = lang || "en-US"; u.rate = rate || 0.9; speechSynthesis.speak(u); } catch (e) {}
  }
  ...
  function inferStructure(d) {
    ...
    if (/\b(have|has)\b\s+\w+/.test(s)) {
      ...
      formula = "have/has + V3";
      focus = "Geçmişte başlayıp şimdiyle bağlantılı durum";
      rule = "I/you/we/they için have, he/she/it için has kullanılır; fiil V3 olur.";
    } else if (/\b(am|is|are)\b\s+\w+ing\b/.test(s)) {
      name = "Present Continuous";
      formula = "am/is/are + V-ing";
      focus = "Şu anda devam eden eylem";
      rule = "Özneye göre am/is/are seçilir; fiile -ing eklenir.";
    } else if (/\b(was|were)\b\s+\w+ing\b/.test(s)) {
      name = "Past Continuous";
      formula = "was/were + V-ing";
      focus = "Geçmişte devam eden eylem";
      rule = "I/he/she/it için was, you/we/they için were kullanılır.";
    } else if (/\bwill\b/.test(s)) {
      name = "Future Simple";
      formula = "will + V1";
      focus = "Gelecek karar/tahmin";
      rule = "Will'den sonra fiilin yalın hâli gelir.";
    } else if (/\b(can|could|should|must|may|might)\b/.test(s)) {
      name = "Modal Verb";
      formula = "modal + V1";
      focus = "Yetenek, tavsiye, zorunluluk veya ihtimal";
      rule = "Modal fiilden sonra fiilin yalın hâli gelir.";
    } else if (/\b(did|didn't|was|were|went|came|saw|took|made|had|\w+ed)\b/.test(s)) {
      name = "Past Simple";
      formula = "V2 / did + V1";
      focus = "Geçmişte bitmiş eylem";
      rule = "Olumlu cümlede V2; soru/olumsuzda did + V1 kullanılır.";
    } else if (/\b(do|does|don't|doesn't|always|usually|often|every)\b/.test(s)) {
      name = "Present Simple";
      formula = "V1 / he-she-it + V-s";
      focus = "Rutinler ve genel doğrular";
      rule = "He/she/it ile olumlu cümlede fiile çoğu zaman -s gelir.";
    }
  ...
  function similarityScore(a, b) {
    ...
    let hit = 0;
    const used = new Set();
    A.forEach(w => { const i = B.findIndex((x, idx) => !used.has(idx) && x === w); if (i >= 0) { used.add(i);
        hit++; } });
  ...
  function openAITest() {
    ...
    wdBindPromptEditor(ov);
    const step = ov.querySelector("#wdQuizStep"),
      fb = ov.querySelector("#wdQuizFeedback"),
      summary = ov.querySelector("#wdQuizSummary");
    ...
    function render() {
      ...
      step.innerHTML = html;
      step.querySelectorAll("[data-choice]").forEach(b => b.onclick = function() {
        step.querySelectorAll(".wd-choice").forEach(x => x.classList.remove("ok", "bad"));
        selected = this.dataset.choice;
        this.classList.add("ok");
      });
      const show = step.querySelector("#wdQuizShowAnswer");
      if (show) show.onclick = function() { const ta = step.querySelector("#wdQuizTextAnswer"); if (ta) ta.value = q.answer; };
    }
    ...
    async function check() {
      ...
      if (q.type === "choice") {
        ...
        pct = structureHintScore(val, st);
        ok = pct >= 60;
      } else {
        pct = similarityScore(q.answer, val);
        ok = pct >= 70;
      }
      let aiText = null;
    ...
    function next() {
      ...
      if (current >= qs.length) {
        ...
        summary.querySelector("#wdOpenSimilarAfterQuiz").onclick = function() { openSimilarSentences(); };
        return;
      }
      render();
    }
  ...
  async function openSimilarSentences() {
    ...
    function renderRows(rows) {
      ...
      out.querySelectorAll("[data-say]").forEach(b => b.onclick = function() { speak(this.dataset.say, "en-US", 0.88); });
    }
    ov.querySelector("#wdGenerateSimilar").onclick = async function() {
      ...
      if (ai) {
        try { const m = ai.match(/\[[\s\S]*\]/);
          rows = JSON.parse(m ? m[0] : ai); } catch (e) {}
      }
      renderRows(Array.isArray(rows) && rows.length ? rows.slice(0, 5) : offlineSimilar());
    };
    ov.querySelector("#wdSimilarListenAll").onclick = function() {
      const text = [...out.querySelectorAll("[data-say]")].map(b => b.dataset.say).join(". ");
      if (text) speak(text, "en-US", 0.84);
    };
  ...
  function openStory() {
    ...
    ov.querySelector("#wdStoryListen").onclick = function() { speakMixed(this.parentElement.parentElement.querySelector("#wdStoryOut").innerText); };
  }
  ...
  function openPodcast() {
    ...
    ov.querySelector("#wdGenPodcast").onclick = async function() {
    ...
    ov.querySelector("#wdPodPlay").onclick = function() { speakMixed(this.parentElement.parentElement.querySelector("#wdPodcastOut").innerText); };
    ov.querySelector("#wdPodStop").onclick = function() { speechSynthesis.cancel(); };
  }
  ...
  function openConversation() {
    ...
    async function aiReply(userText, first) {
    ...
    ov.querySelector("#wdStart").onclick = function() { aiReply("", true); };
    ov.querySelector("#wdSend").onclick = function() {
    ...
    ov.querySelector("#wdMic").onclick = function() { listen(function(t) { ov.querySelector("#wdChatInput").value = t; }); };
    aiReply("", true);
  }
  ...
  function openWriting() {
    ...
    wdBindPromptEditor(ov);
    ov.querySelector("#wdWriteMic").onclick = function() { listen(function(t) { ov.querySelector("#wdWriteInput").value = t; }); };
    ov.querySelector("#wdWriteRead").onclick = function() { speakMixed(this.parentElement.parentElement.querySelector("#wdWriteOut").innerText); };
    ov.querySelector("#wdCheckWriting").onclick = async function() {
  ...
  function openPartner() {
    ...
    function add(who, text) { const div = document.createElement("div");
      div.className = "wd-msg " + (who === "user" ? "wd-user" : "wd-ai");
      div.innerHTML = esc(text);
      chat.appendChild(div); }
    add("ai", `Sure. Try to use this sentence: "${d.sentence}"`);
    ov.querySelector("#wdPartnerSend").onclick = async function() {
      const val = clean(ov.querySelector("#wdPartnerInput").value);
      if (!val) return;
      add("user", val);
      ov.querySelector("#wdPartnerInput").value = "";
      ...
      const reply = ai || "Nice. Can you say it one more time in a different way?";
      add("ai", reply);
      speakMixed(reply);
    };
  }
  ...
  No text on image. Natural lighting.`;
    ...
    ov.querySelector("#wdCopyPrompt").onclick = async function() { try { await navigator.clipboard.writeText(prompt);
        alert("Kopyalandı"); } catch (e) {} };
    ov.querySelector("#wdSearchUnsplash").onclick = function() { window.open("https://unsplash.com/s/photos/" + encodeURIComponent(tokens(d.sentence).slice(0, 5).join(" ")), "_blank"); };
  }
  ...
  function openShadow() {
    ...
    ov.querySelector("#wdShadowListen").onclick = function() { speak(d.sentence, "en-US", 0.9); };
    ov.querySelector("#wdShadowSlow").onclick = function() { speak(d.sentence, "en-US", 0.62); };
    ov.querySelector("#wdShadowMic").onclick = function() {
      const SR = window.SpeechRecognition || window.webkitSpeechRecognition;
      if (!SR) { alert("Bu tarayıcıda konuşma tanıma yok. Chrome/Edge kullan."); return; }
      ...
      r.onend = function() { stopRecordingBlob(); };
      try { r.start(); } catch (e) { status.innerHTML = "❌ Mikrofon başlatılamadı.";
        stopRecordingBlob(); }
    };
    replay.onclick = function() { if (recUrl) { const a = new Audio(recUrl);
        a.play(); } };
    ov.querySelector("#wdShadowReset").onclick = function() {
  ...
  function enhance() {
    ...
    `;
    const map = {
      translate: openGoogleTranslate,
      shadow: openShadow,
      test: openAITest,
      similar: openSimilarSentences,
      story: openStory,
      podcast: openPodcast,
    ...
    row.querySelectorAll("[data-wd]").forEach(b => b.onclick = function() { map[this.dataset.wd](); });
  ...
  function schedule() { clearTimeout(t);
