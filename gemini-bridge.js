...
(function(global){
 ...
 function mount(){
  ...
  var b=document.getElementById("dhTeacherSentenceReturn");if(!b){b=document.createElement("a");b.id="dhTeacherSentenceReturn";b.textContent="← Çalışılan cümleye dön";document.body.appendChild(b);}b.href=ret;b.onclick=function(ev){ev.preventDefault();try{localStorage.removeItem("teacherReturnURL");sessionStorage.removeItem("teacherReturnURL");}catch(e){}location.assign(ret);};
  var old=document.getElementById("teacherBack");if(old){old.href=ret;old.textContent="← Cümleye dön";old.title="Modülde çalışılan cümleye dön";old.classList.add("teacher-return-active");old.onclick=b.onclick;old.style.setProperty("display","flex","important");old.style.setProperty("visibility","visible","important");}
 }
