...
(function(global){
 ...
 function goBackToSentence(ev){
  var node=ev.target&&ev.target.closest&&ev.target.closest("#teacherBack,#dhTeacherSentenceReturn,[data-dh-sentence-return='1']");if(!node)return;var ret=savedReturn();if(!ret)return;
 ...
 function mount(){
  ...
  var old=document.getElementById("teacherBack");if(old){old.href=ret;old.textContent="← Cümleye dön";old.title="Modülde çalışılan cümleye dön";old.classList.remove("back");old.classList.add("teacher-return-link","teacher-return-active");old.setAttribute("data-dh-sentence-return","1");old.onclick=goBackToSentence;old.style.setProperty("display","flex","important");old.style.setProperty("visibility","visible","important");}
 }
