...
function css(){
  ...
  +".dhgb-send{background:linear-gradient(180deg,#059669,#047857);color:#fff}"
  ...
  +".dhgb-tog{background:rgba(96,165,250,.08);border:1px solid rgba(96,165,250,.5);border-radius:9px;color:#93c5fd;font-size:12px;font-weight:800;cursor:pointer;padding:7px 12px;margin:0 0 10px;text-decoration:none}.dhgb-tog:hover{background:rgba(96,165,250,.16);color:#dbeafe}";
...
function explanationSectionKey(label){
  ...
  var aliases={"YAPI VE KALIPLAR":"YAPIKALIPLAR","HANGİ DURUMDA DOĞAL":"DURUM","DAHA SADE NASIL SÖYLERİM":"SADE","HANGİSİNİ KULLANMALIYIM":"TERCIH","BENZER ÖRNEKLER":"BENZER","KISA KONUŞMA":"KONUSMA","SIRA SENDE":"ALISTIRMA","ÖRNEK CEVAP":"CEVAP","ANLAM":"ANLAM","DOĞAL ANLAM":"ANLAM","YAPI":"YAPI","DİLBİLGİSİ":"YAPI","KALIPLAR":"KALIPLAR","ÖNEMLİ KALIPLAR":"KALIPLAR","TELAFFUZ":"TELAFFUZ","YAYGIN HATALAR":"HATALAR","HATALAR":"HATALAR","ÖRNEKLER":"ORNEKLER","ORNEKLER":"ORNEKLER"};
  return aliases[key]||"";
}
...
function highlightEnglish(text){
  var s=String(text||""),out="",last=0,re=/\[([^\[\]]+)\]|"([^"]+)"|“([^”]+)”|\(([^()]+)\)/g,m;
  while((m=re.exec(s))){
    out+=esc(s.slice(last,m.index));
    if(m[1]!==undefined)out+='<b class="dh-exp-en">'+esc(m[1])+'</b>';
    else if(m[2]!==undefined)out+='<span class="dh-exp-quote">"'+esc(m[2])+'"</span>';
    else if(m[3]!==undefined)out+='<span class="dh-exp-quote">“'+esc(m[3])+'”</span>';
    else out+='<span class="dh-exp-paren">('+esc(m[4])+')</span>';
    last=re.lastIndex;
  }
...
function formatExplanation(input){
  ...
  var order=["ANLAM","YAPIKALIPLAR","YAPI","KALIPLAR","DURUM","SADE","TERCIH","BENZER","KONUSMA","TELAFFUZ","HATALAR","ORNEKLER","ALISTIRMA","CEVAP"],titles={"YAPIKALIPLAR":"YAPI VE KALIPLAR","DURUM":"HANGİ DURUMDA DOĞAL","SADE":"DAHA SADE NASIL SÖYLERİM","TERCIH":"HANGİSİNİ KULLANMALIYIM","BENZER":"BENZER ÖRNEKLER","KONUSMA":"KISA KONUŞMA","ALISTIRMA":"SIRA SENDE","CEVAP":"ÖRNEK CEVAP",ANLAM:"Bağlamdaki doğal anlam",YAPI:"Cümle yapısı ve dilbilgisi",KALIPLAR:"Önemli kalıplar",TELAFFUZ:"Telaffuz ve akıcı konuşma",HATALAR:"Türk öğrenciler için yaygın hatalar",ORNEKLER:"Doğal örnekler"};
  ...
  function prose(lines){
    ...
    if(list)return'<ul class="dh-exp-list">'+rows.map(function(x){return'<li>'+highlightEnglish(x.replace(/^[-•]\s+/,""))+'</li>'}).join("")+'</ul>';
    return rows.map(function(x){return'<p>'+highlightEnglish(x)+'</p>'}).join("");
  }
