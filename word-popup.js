...
(function(global){
  ...
  var __wpLastT=0, __wpLastX=0, __wpLastY=0;
  function onDocClick(e){
    if(!enabled || popEl) return;
    var t=e.target; if(!t) return;
    if(t.closest && t.closest("input,textarea,button,a,select,.no-wordpop")) return;
    /* ÇİFT TIKLAMA / ÇİFT DOKUNMA şartı: tek tık popup AÇMAZ. İkinci tık 350 ms
       içinde ve ~30 px yakınında olmalı. (dblclick yerine el ile: mobil çift
       dokunma da masaüstü çift tık da tek yolla yakalanır.) */
    var nowT=Date.now(), cx=e.clientX||0, cy=e.clientY||0;
    var cift = (nowT-__wpLastT<350) && Math.abs(cx-__wpLastX)<30 && Math.abs(cy-__wpLastY)<30;
    __wpLastT=nowT; __wpLastX=cx; __wpLastY=cy;
    if(!cift) return;
    __wpLastT=0;                                  // açıldı: üçüncü tık yeni çift saymasın
    var sel=(global.getSelection && global.getSelection().toString())||"";
    if(sel && sel.length>40) return;              // uzun seçim (kopyalama) popup açmasın
