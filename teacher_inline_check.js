...
function getSelectedTeacher(){
  ...
  const fromUrl = (p.get("teacher") || p.get("avatar") || "").trim();
  if (fromUrl === "teacher1" || fromUrl === "teacher2") {
    try { localStorage.setItem("selectedTeacherAvatar", fromUrl); } catch {}
    return fromUrl;
  }
  return localStorage.getItem("selectedTeacherAvatar") || "teacher1";
}
function teacherDir(){
  return "./assets/avatars_v3/" + getSelectedTeacher() + "/";
}
...
function pickVoiceForLang(lang){
  refreshVoices();
  const voices = _voiceCache || [];
  const wantTr = /^tr/i.test(lang || "");
  if(wantTr){
    return voices.find(v=>/^tr/i.test(v.lang||"")) ||
           voices.find(v=>/turkish|türk|turk/i.test(v.name||"")) ||
           null;
  }
  const en = voices.filter(v=>/^en/i.test(v.lang||""));
  if(!en.length) return null;
  const male = /(male|david|mark|george|daniel|james|john|alex|fred|thomas|guy|brian|ryan|matthew|arthur|oliver)/i;
  const pref = en.filter(v=>/en-US|en_GB|en-GB|en_US/i.test(v.lang||""));
  return pref.find(v=>male.test(v.name||"")) || pref[0] || en[0];
}
...
function splitLongTurkish(text){
...
function buildMixedSpeechSegments(text){
  text = String(text||"")
    .replace(/[“”]/g,'"')
    .replace(/[‘’]/g,"'")
    .replace(/\r/g,"\n");
  ...
  while((m = re.exec(text))){
    ...
    splitLongTurkish(before).forEach(t=>segments.push({text:t, lang:"tr-TR"}));

    const en = cleanForSpeech(m[1] || m[2] || "");
    if(en) segments.push({text:en, lang:"en-US"});

    last = re.lastIndex;
  }

  splitLongTurkish(text.slice(last)).forEach(t=>segments.push({text:t, lang:"tr-TR"}));

  // Bazı Groq cevaplarında İngilizce örnekler tırnaksız gelebiliyor.
  // Tamamen İngilizce gibi görünen kısa satırları İngilizceye çevir.
  return segments.map(seg=>{
    const s = seg.text;
    const latin = (s.match(/[A-Za-z]/g)||[]).length;
    const turkish = (s.match(/[çğıöşüÇĞİÖŞÜ]/g)||[]).length;
    const englishHints = /\b(i|you|he|she|we|they|am|is|are|was|were|do|does|did|can|could|would|should|not|hungry|breakfast|hello|welcome|reservation|please|mr|mrs|the|a|an|to|from|with|for)\b/i.test(s);
    if(seg.lang==="tr-TR" && latin > 0 && turkish === 0 && englishHints && s.length < 120){
      return {text:s, lang:"en-US"};
    }
    return seg;
  }).filter(seg=>seg.text);
}
...
function speakOne(text, lang, runId){
  return new Promise(resolve=>{
    text = cleanForSpeech(text);
    if(!text || runId !== _ttsRun){ resolve(); return; }
...
function speakUtterance(text, lang, runId){
  return new Promise(resolve=>{
    text = cleanForSpeech(text);
    if(!text || runId !== _ttsRun){ resolve(); return; }
    ...
    const finish = ()=>{
      if(settled) return;
      settled = true;
      if(runId === _ttsRun && TeacherAvatar) TeacherAvatar.stop();
    ...
    try{
      ...
      u.rate = /^tr/i.test(lang) ? 0.92 : 0.95;
      u.pitch = /^tr/i.test(lang) ? 0.92 : 0.78;

      u.onstart = ()=>{
        if(runId === _ttsRun && TeacherAvatar){
          // Her parça için avatarı yeniden başlatıyoruz.
          // Böylece uzun açıklamada ses devam ederken avatar erken susmaz.
          TeacherAvatar.speakText(text, estimateSpeechDuration(text) + 9000);
        }
      };
      u.onend = finish;
      u.onerror = finish;
      ...
      // Bazı mobil tarayıcılarda onend gelmeyebiliyor.
      // Uzun bir güvenlik süresi bırakıyoruz ama normalde onend bitirir.
      setTimeout(finish, estimateSpeechDuration(text) + 12000);
    }catch{
      finish();
    }
  });
...
async function speakSegments(segments){
  ...
  for(const seg of segments){
    if(runId !== _ttsRun) break;
    await speakUtterance(seg.text, seg.lang, runId);
  }
  if(runId === _ttsRun && TeacherAvatar) TeacherAvatar.stop();
}
function speak(text, lang){
  speakSegments([{text:String(text||""), lang:lang || "tr-TR"}]);
}
function speakEnglish(text){ speakSegments([{text:String(text||""), lang:"en-US"}]); }
function speakTurkish(text){ speakSegments([{text:String(text||""), lang:"tr-TR"}]); }
function speakMixed(text){ speakSegments(buildMixedSpeechSegments(text)); }
...
function renderExplanation(reply){
  ...
  setOut(`
    ...
    <div class="explain-actions">
      <button class="explain-play" id="explainListenBtn">🔊 Detaylı açıklamayı dinle</button>
      <button class="explain-play" id="sentenceListenBtn">🔊 Cümleyi dinle</button>
      <button class="explain-play" id="stopListenBtn">⏹ Durdur</button>
    </div>
    <div class="followup">
  ...
  document.getElementById("explainListenBtn").onclick = () => speakMixed(State.explanation);
  document.getElementById("sentenceListenBtn").onclick = () => speakEnglish(State.en);
  document.getElementById("stopListenBtn").onclick = () => stopSpeech();
}
...
async function askFollowup(q){
  ...
  try{
    const reply = await groqChat(State.history);
    State.history.push({ role:"assistant", content:reply });
    qa.innerHTML = `<div class="qa-q">❓ ${esc(q)}</div><div class="explain">${renderRich(reply)}</div><div class="explain-actions"><button class="explain-play qaListen">🔊 Yanıtı dinle</button></div>`;
    const btn = qa.querySelector(".qaListen");
    if(btn) btn.onclick = () => speakMixed(reply);
  }catch(e){
