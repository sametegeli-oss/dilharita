...
// ==========================================
// DILHARITA - APP.JS (IndexedDB + Manuel Yapıştırma Kutulu Tam Sürüm)
// ==========================================
...
function initDB() {
    ...
    return new Promise((resolve, reject) => {
        const request = indexedDB.open("DilHaritaAI_DB", 2);
...
async function getAIExplanationFromDB(sentence) {
    try {
        ...
        return new Promise((resolve, reject) => {
            ...
            request.onsuccess = (event) => resolve(event.target.result ? event.target.result.explanation : null);
            request.onerror = (event) => reject(event.target.error);
        });
    } catch (e) { return null; }
}
...
async function saveAIExplanationToDB(sentence, explanation) {
    try {
        ...
        return new Promise((resolve, reject) => {
            ...
            const store = transaction.objectStore("ai_explanations");
            const request = store.put({ sentence, explanation, timestamp: new Date().toISOString() });
...
// --- AI'ye Sor ve Manuel Yapıştırma Kutusu Bileşeni ---
function Fe({item:e,onWordClick:t,onGrade:n,graded:r}){
    ...
    l.useEffect(() => {
        if (!e || !e.en) return;
        // Kart değiştiğinde IndexedDB'den kontrol et
        getAIExplanationFromDB(e.en).then(cached => {
            if (cached) {
                setAiExplanation(cached);
                setAiSourceTag(`🤖 AI Açıklaması (IndexedDB'den otomatik yüklendi)[cite: 1]`);
                setShowPasteBox(!1);
            } else {
                setAiExplanation(``);
                setAiSourceTag(``);
    ...
    async function handleAISor() {
        ...
        // Yoksa Gemini'yi aç ve yapıştırma kutusunu göster
    ...
    return e ? _jsxs(`div`,{className:`card`,children:[
        ...
        _jsxs(`div`,{className:`card-actions`,children:[
            ...
            _jsx(`button`,{className:`btn dh-aiask-btn`,style:{background:`linear-gradient(135deg,#7c3aed,#4338ca)`,color:`#fff`,fontWeight:800},onClick:handleAISor,children:`🤖 AI'ye Sor`}),
            _jsx(`button`,{className:`btn btn-ghost`,onClick:()=>a(e=>!e),children:i?`Detayı gizle`:`Detay`})
        ]}),

        // Gemini Cevabını Yapıştırma Kutusu (Eğer DB'de yoksa açılır)
        showPasteBox && _jsxs(`div`,{className:`ai-paste-box`,style:{marginTop:`12px`,padding:`12px`,background:`rgba(30,41,59,0.9)`,borderRadius:`10px`,border:`1px solid #7c3aed`},children:[
            _jsx(`div`,{style:{fontSize:`12px`,color:`#c4b5fd`,marginBottom:`6px`,fontWeight:700},children:`Gemini'den kopyaladığınız açıklamayı buraya yapıştırın:`}),
            _jsx(`textarea`,{value:manualText,onChange:ev=>setManualText(ev.target.value),placeholder:`Cevabı buraya yapıştırıp kaydet butonuna basın...`,style:{width:`100%`,height:`80px`,background:`#0f172a`,color:`#fff`,border:`1px solid rgba(255,255,255,0.2)`,borderRadius:`6px`,padding:`8px`,fontSize:`13px`,resize:`vertical`}}),
            _jsxs(`div`,{style:{display:`flex`,gap:`8px`,marginTop:`8px`,justifyContent:`flex-end`},children:[
                _jsx(`button`,{className:`btn`,style:{padding:`6px 12px`,fontSize:`12px`,background:`#334155`,color:`#fff`},onClick:()=>setShowPasteBox(!1),children:`İptal`}),
                _jsx(`button`,{className:`btn btn-primary`,style:{padding:`6px 12px`,fontSize:`12px`,background:`#7c3aed`,color:`#fff`},onClick:handleSaveManual,children:`Kaydet ve IndexedDB'ye Ekle`})
            ]})
        ]}),

        // Kaydedilmiş AI Açıklaması ve Etiket Alanı
