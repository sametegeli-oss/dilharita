...
function get(type,input,prompt){var id=identity(type,input),r=all()[id];if(!r||!r.text)return null;try{if(global.DHProviders&&DHProviders.showSource)DHProviders.showSource(r.source||{provider:"unknown",label:"AI",model:"Eski kayıtta model bilgisi yok",at:r.updatedAt||r.createdAt},true);}catch(e){}return{record:r,promptChanged:!!(r.promptHash&&r.promptHash!==promptHash(prompt))};}
function put(type,input,prompt,text,label,source){text=String(text||"").trim();if(!text)return null;var db=all(),id=identity(type,input),now=Date.now();if(!source)try{source=global.DHProviders&&DHProviders.lastResponseInfo;}catch(e){}db[id]={id:id,type:String(type||"general"),inputHash:id.split(":").slice(1).join(":"),promptHash:promptHash(prompt),text:text,label:String(label||""),source:source||null,createdAt:(db[id]&&db[id].createdAt)||now,updatedAt:now};save(db);return db[id];}
