...
ok(/addEventListener\("click", onDocClick, true\)/.test(wp), "yakalama fazında dinliyor");
...
console.log("\n--- sözlükte yoksa AI ---");
ok(/if\(entry\) open\(entry\); else defineWithAI\(word\)/.test(wp),
...
ok(/IDB\.get\(word\)/.test(dwa), "önce yerel önbelleğe bakıyor (gereksiz AI çağrısı yok)");
...
ok(wpVar.length>=35, `${wpVar.length} sayfada yüklü`);
const gerekli=["chatdoctor.html","chatteacher1.html","hata-defteri.html","phrasal-verbs.html",
               "pdfoku.html","ocr-sentence.html","sesdalga.html","index.html","teacher1.html"];
...
ok(!fs.readFileSync("index-app.html","utf8").includes("word-popup.js"),
   "index-app.html HARİÇ — kendi kelime araçları var, çakışma riski");
