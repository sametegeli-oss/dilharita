...
function practiceExplanationRows(data){var rows=[],examples=practiceExamplesStore();(data&&data.segments||[]).forEach(function(x,i){if(x.transcriptEN&&!hasFivePracticeExamples(examples[keyOf(x)]))rows.push({index:i,id:"DH-E"+String(i+1).padStart(4,"0"),en:x.transcriptEN,tr:x.translationTR||""})});return rows}
...
async function explainAllWithPracticeWorkflow(){if(!study)return;var rows=practiceExplanationRows(study);if(!rows.length){setStatus("Tüm cümlelerde 5 uygulama cümlesi kayıtlı.","ok");return}if(!global.DHGemini||!DHGemini.ask){setStatus("Gemini kopyala-yapıştır köprüsü yüklenmedi.","error");return}DHGemini.ask({title:"5 uygulama cümlesi · "+rows.length+" cümle kaldı",providerName:"Gemini",openUrl:DHGemini.url,prompt:practiceExamplesBatchPrompt(rows,study.title,study.level),hint:"Gemini cevabı birkaç mesaja bölerse hepsini toplayıp TEK bir JSON hâlinde buraya yapıştırın. Her text alanı 1’den 5’e kadar eksiksiz örnek içermelidir.",resume:{type:"youtube-practice-examples",videoId:videoId},parse:function(t){return parsePracticeExplanationBatchReply(String(t||"").replace(/^\s*DH-ID:[^\n]*\n/i,"").trim(),rows)},onResult:function(r){updateExplanationButton();scheduleSave();backupYouTubeNow();var remain=practiceExplanationRows(study).length;setStatus(r.applied+"/"+r.total+" cümleye 5 uygulama örneği kaydedildi."+(r.skipped?" "+r.skipped+" eksik yanıt atlandı.":"")+(remain?" "+remain+" cümle kaldı; “5 örnek üret”e tekrar basın.":" Hepsi tamamlandı."),"ok")},onCancel:function(){}})}
