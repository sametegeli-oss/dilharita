...
(function(global){
"use strict";
var studyApi=null,study=null,record=null,videoId="",videoUrl="",player=null,playerReady=null,ytApiReady=null,tick=null,active=-1,loopOn=false,loopIndex=-1,seekNonce=0,showEN=true,showTR=true,captionOn=true,muted=false,searchText="",saving=null,recognition=null,shadowTimer=null,studyMode="watch",autoPause=false,karaokeOn=true,autoPausedIndex=-1,selectedWord="",mediaRecorder=null,mediaStream=null,recordedChunks=[],recordedAudioUrl="",recordedAudioBlob=null,shadowStartedAt=0,shadowPlaybackAudio=null,shadowSyncTimer=null,shadowSyncOn=false,ownVoiceOn=false,ownVoiceRecords={},ownVoiceAudio=null,ownVoiceUrl="",ownVoiceKey="",ownVoiceLastTime=-1,guideVoiceOn=false,guideVoiceKey="",guideVoiceLastTime=-1,guideUtterance=null,guideLastStartedAt=0,guideLastStartedKey="",micAudioContext=null,micSource=null,micAnalyser=null,micMeterFrame=0,micMeterData=null,shadowSignalTimer=null,shadowEndTimer=null,shadowDurationTimer=null,shadowStopHandler=null,shadowTargetSeconds=0,ttsFallback=false,ttsClock=null,ttsCurrent=0,ttsDuration=0,ttsPlaying=false,ttsLastSegment=-1,ttsUtterance=null;
...
function updatePlayButton(){var b=$("playToggle"),playing=ttsFallback?ttsPlaying:false;try{if(!ttsFallback)playing=player&&global.YT&&player.getPlayerState()===YT.PlayerState.PLAYING}catch(e){}if(b)b.textContent=playing?"❚❚":"▶"}
function playPause(){if(!player)return;try{if(studyMode==="shadow")forceShadowMute();else if(ownVoiceOn||guideVoiceOn)forceOwnVoiceMute();var playing=ttsFallback?ttsPlaying:(global.YT&&player.getPlayerState()===YT.PlayerState.PLAYING);playing?player.pauseVideo():player.playVideo();updatePlayButton()}catch(e){}}
...
async function analyze(id,url,requestedLanguage){var submit=$("videoForm").querySelector('button[type="submit"]');submit.disabled=true;setStatus("Video bilgileri YouTube’dan doğrulanıyor…","loading");videoId=id;videoUrl=url;try{var meta=await metaFromPlayer(id);pendingTranscriptMeta=meta;setStatus(meta.durationSeconds?time(meta.durationSeconds)+" video doğrulandı. YouTube konuşma metnini zamanlarıyla yapıştırın.":"Video bulundu. Süre, yapıştıracağınız son transkript zamanından hesaplanacak.","ok");openTranscriptPaste(meta)}catch(e){setStatus(e&&e.message?e.message:"Video doğrulanamadı.","error")}finally{submit.disabled=false}}
